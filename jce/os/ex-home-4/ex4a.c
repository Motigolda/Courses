...
#define STR_WAIT_TO_USER_INPUT "P(x), x0: "
...
int64_t CalculatePolynomial(polynomial_t *polynomial, uint32_t solve_for_x);
...
task_t *ParseUserInput(char *line_from_user){
    ...
    uint32_t i = 0;
    uint32_t buff_ind = 0;
    char buffer[INPUT_LINE_MAX_LENGTH] = { 0 };
...
int64_t CalculatePolynomial(polynomial_t *polynomial, uint32_t solve_for){
    return 0;
}
