...
void AppendEntryToHistory(history_t *history, char *entry){
    ...
    size_t current_histry_string_len = strlen(history->history_string);
    size_t new_entry_len = strlen(entry); 
    
    history->history_string = realloc(history->history_string, current_histry_string_len + new_entry_len + 1);
    
    if (history->history_string[current_histry_string_len - 1] != '\n')
